  ...
  return (
    <TiltWrapper delay={index * 0.04}>
      <article className="h-full rounded-[24px] border border-border bg-surface p-5 soft-glow card-hover">
        ...
        <div className="mt-4 text-[1.7rem] font-bold leading-none tracking-tight text-text tabular-nums">
          {item.value}
        </div>
        <div className={`mt-2 flex items-center gap-1 text-xs font-medium ${isUp ? "text-success" : "text-danger"}`}>
