...
export function RechartsChart({ type, height, series, options }: RechartsChartProps) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="rounded-2xl border border-white/15 bg-slate-950/80 p-3.5 shadow-2xl backdrop-blur-xl">
          {label && <p className="text-[11px] font-semibold uppercase tracking-wider text-text-soft mb-1.5">{label}</p>}
            ...
            {series.map((s, idx) => {
              ...
              return (
                <Line
                  ...
                  dot={{ r: 3, strokeWidth: 1, stroke: "#030712", fill: colors[idx % colors.length] }}
